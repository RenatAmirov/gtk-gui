...
uiDef =
  ...
  \      <menuitem name=\"Body\" action=\"BodyAction\" />\
  ...
  \      <toolitem name=\"Body\" action=\"BodyAction\"/>\
  \      <separator/>\
  \    </placeholder>\
...
main = do
  ...
  bodyAct <- actionNew "BodyAction" (__"Body")
            (Just (__"Надо подкачаться!!!"))
            (Just stockSave)
  on bodyAct actionActivated $ putStrLn "Body activated."
  ...
  mapM_ (\act -> actionGroupAddActionWithAccel standardGroup act (Nothing::Maybe Text))
    [newAct, openAct, saveAct, bodyAct, saveAsAct, exitAct, cutAct, copyAct, pasteAct]
